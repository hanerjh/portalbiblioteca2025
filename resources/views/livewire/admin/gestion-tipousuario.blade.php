<div>
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <button wire:click="create()" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i> Nuevo Tipo usuario</button>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @forelse ($tipoUsuario  as $tpu)
                        <tr>
                            <td>{{ $tpu->nombre }}</td>
                            <td>{{ $tpu->color_fondo}}</td>
                             <td>{{ $tpu->siglas}}</td>
                            <td><span class="badge" style="background-color: {{ $tpu->color_fondo }}; color: {{ $tpu->color_texto }};">{{ $tpu->activo ? 'Activo' : 'Inactivo' }}</span></td>
                            <td class="text-end">                                
                                <button wire:click="edit({{ $tpu->id }})" class="btn btn-sm btn-info" title="Editar"><i class="bi bi-pencil"></i></button>
                                <button wire:click="delete({{ $tpu->id }})" wire:confirm="¿Estás seguro?" class="btn btn-sm btn-danger" title="Eliminar"><i class="bi bi-trash"></i></button>
                            </td>
                        </tr>
                        @empty
             ...
             <div class="mt-3">{{ $tipoUsuario ->links() }}</div>
        </div>
    </div>
